...
module.exports = function setupHook({
  ...
  const isException = buildExceptionChecker(ignore, only);
 ...
 * @return {function}
 */
function buildExceptionChecker(ignore, only) {
  const param = ignore || only;
  let fn;
  ...
  if (param instanceof RegExp) {
    fn = filepath => param.test(filepath);
  } else if (typeof param === 'string') {
    fn = filepath => globToRegex(param).test(filepath);
  } else {
    fn = param;
  }
